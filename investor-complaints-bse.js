(function(){
  ...
  function fmtDate(v){var d=parsed(v);if(!d)return v?esc(v):'—';return new Intl.DateTimeFormat('en-IN',{day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit',hour12:true,timeZone:'Asia/Kolkata'}).format(d);}
  ...
  function setView(){
    ...
    var list=document.getElementById('feedList');if(list){list.hidden=false;list.className='feed-list results-wrap';}
  ...
  function render(items){
    ...
    var rows=(items||[]).slice().sort(function(a,b){return (parsed(b&&b.date)||0)-(parsed(a&&a.date)||0);});
    var body=rows.length?rows.map(function(item){
      ...
      return '<tr><td>'+esc(fy(q,item))+'</td><td>'+qcell+'</td><td>'+esc(item&&item.status||'New')+'</td><td>'+fmtDate(item&&item.date)+'</td><td>'+fmtDate(item&&item.revisedDate)+'</td><td>'+esc(item&&item.revisionReason||'—')+'</td><td>'+xb+'</td></tr>';
    }).join(''):'<tr><td colspan="7" class="result-empty">No investor-complaint filings were returned by BSE.</td></tr>';
    list.innerHTML='<table class="governance-table"><thead><tr><th>Year</th><th>Quarter</th><th>Status</th><th>Filing Date Time</th><th>Revised Filing Date Time</th><th>Revision Reason</th><th>XBRL Link</th></tr></thead><tbody>'+body+'</tbody></table>';
  }
  function load(){
    setView();
    ...
    }).catch(function(e){console.error(e);var status=document.getElementById('feedStatus');if(status)status.innerHTML='Source: <strong>BSE</strong> · Temporarily unavailable';var list=document.getElementById('feedList');if(list)list.innerHTML='<div class="feed-error">BSE investor complaint data is temporarily unavailable. Please try again shortly.</div>';});
  }
  ...
  function bind(){
    ...
    document.addEventListener('click',function(e){var btn=isComplaintButton(e.target);if(!btn)return;e.preventDefault();e.stopPropagation();load();},true);
