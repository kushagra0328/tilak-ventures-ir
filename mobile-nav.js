(function(){
  ...
  var css = `
    ...
    @media (max-width:900px){
      ...
      .results-table,.annual-table,.voting-table,.shareholding-table,.governance-table,.related-party-table{min-width:760px!important}
    }
  ...
  if(sidebar){
    ...
    if(main){
      ...
      sidebar.querySelectorAll('button[data-category],button[data-query]').forEach(function(btn){
      ...
      select.addEventListener('change',function(){
        ...
        var btn=buttons[Number(select.value)];
        if(btn)btn.click();
      });
